...
export class HomeComponent {
  ...
  getUsers() {
    this.http.get<UserModel[]>("https://localhost:7222/api/Chat/GetUsers").subscribe(res => {
  ...
  changeUser(user: UserModel) {
    ...
    this.http.get(`https://localhost:7222/api/Chat/GetChats?userId=${this.user.id}&toUserId=${this.selectedUserId}`).subscribe((res: any) => {
      this.chats = res;
    });
  }
  ...
  sendMessage() {
    ...
    this.http.post<ChatModel>("https://localhost:7222/api/Chat/SendMessage", data).subscribe(
      (res) => {
        this.chats.push(res);
        this.message = "";
      });
